...
class StrmConvertPro(_PluginBase):
    ...
    # 插件配置项ID前缀
    plugin_config_prefix = "strmconvertpro_"
    # 加载顺序
    plugin_order = 27
    # 可使用的用户级别
    auth_level = 1
    ...
    def __change_api(self, convert_confs: list):
        ...
        for convert_conf in convert_confs:
            if str(convert_conf).count("#") != 2:
                ...
                continue
            source_path = str(convert_conf).split("#")[0]
            old_api_url = str(convert_conf).split("#")[1]
            new_api_url = str(convert_conf).split("#")[2]
            logger.info(f"{source_path} 开始修改strm内容前缀模式")
            self.__to_change(source_path, old_api_url, new_api_url)
            logger.info(f"{source_path} 修改完成束")

    # 暂时只支持结构一致url不同修改
    def __to_change(self, source_path: str, old_api_url: str, new_api_url: str):
        files = self.__list_files(Path(source_path), ['.strm'])
        for f in files:
            logger.debug(f"开始处理文件 {f}")
            try:
